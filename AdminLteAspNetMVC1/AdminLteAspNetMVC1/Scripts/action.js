...
$(function () {
    ...
    $("[data-action=search]").on("click", function () {
        ...
        var identity = searchElement.attr("data-element").split("-")[1];
        if (searchBtn.attr("data-searchclick") != undefined && searchBtn.attr("data-searchclick") == "alert")
        {
            alert(searchBtn.attr("data-alert"));
            return false;
        }
        savePreSearch(identity);
        var callBack =searchBtn.attr("data-callback");
        if (callBack != undefined && callBack != "" && callBack != null) {
            refreshGrid(1, identity, null, function () { eval(callBack) });
        }
        else {
            refreshGrid(1, identity);
        }
        ...
        var onselect = function (selection) {
            ...
            if (target != undefined) {
                if (selection != undefined) {
                    if ($("#" + target).attr("class") == "HiddenChange") {
                        $("#" + target).val(selection.data).trigger('change');
                    }
                    else {
